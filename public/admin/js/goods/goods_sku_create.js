...
function goodsTypeChangeData(){
	...
	//清空规格值
	$(".edit-sku-popup-body article ul").empty();
	
	//还原批量选择选中状态
	$(".edit-sku-popup-body article>div label input").removeAttr("checked");
	
	//显示提示内容
	$(".edit-sku-popup-body article>div .empty-info").text("请选择左侧规格列表").show().siblings().hide();
...
function editSkuData(){
	...
	if($specObj.length > 0){
		...
		for(var i=0;i<$specObj.length;i++){
			...
			var curr_li = $(".js-sku-list-popup li[data-spec-id='" + curr_spec.spec_id + "']");
			var spec_value_id_array = new Array();
			curr_li.children("span").text("[" + curr_spec.value.length + "/" + curr_li.attr("data-spec-value-length") + "]");
			for(var j=0;j<curr_spec.value.length;j++){
				spec_value_id_array.push(parseInt(curr_spec.value[j].spec_value_id));
			}
			//添加框体选中
			curr_li.addClass("selected");
			curr_li.attr("data-spec-value-id-array",JSON.stringify(spec_value_id_array));
		}
	}
...
//修改商品时 更新temp_obj
function updateTempObjData(){
	var total_stock  =0;
	if($specObj.length > 0){
		if(sku_list!=null){
			$.each(sku_list,function(c,v){
					...
					html += '<article class="goods-sku-item">';
						...
						switch(parseInt(curr_spec_value.spec_show_type)){
							...
							case 2:
								...
								html += '&nbsp;<i></i>&nbsp;';
								html += '<div>';
								html += '<input type="color" class="input-common-color" name="goods_spec_value'+(i+j)+'" value="'+curr_spec_value.spec_value_data+'">';
								html += '</div>';
								break;
							case 3:
								//图片
