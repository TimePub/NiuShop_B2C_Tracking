...
function init(){
	...
	var presell_money = parseFloat($("#hidden_presell_money").val());
	
	presell_money = presell_money.toFixed(2);
	if(!is_full_payment){
...
function submitOrder() {
	if(validationOrder()){
		...
		$.ajax({
			url : __URL(APPMAIN + "/order/presellOrderCreate"),
			type : "post",
			data : {
...
$('input[name="is_full_payment"]').click(function(){
	...
	is_full_payment = $('input[name="is_full_payment"]:checked').val();
	is_full_payment = is_full_payment == 1 ? true : false;
	$('input[name="is_full_payment"]').parents().removeClass('select');
	var i_obj = $(this).parents();
	i_obj.addClass('select');
	init();
})
