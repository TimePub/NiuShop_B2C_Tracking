...
$(function(){
	...
	$(".mask-layer-delivery-time footer .btn-confirm").click(function(){
		...
		$("#delivery-time .item>span").text(selected.attr("data-text"));
	...
	$(".btn-jiesuan").click(function(){
		if(validationOrder()){
			...
			$.ajax({
				url : __URL(SHOPMAIN + "/order/createComboPackageOrder"),
				type : "post",
				data : {
					...
					'combo_package_id' : $("#hidden_combo_package_id").val(),
					'buy_num' : $("#hidden_buy_num").val(),
...
function init(){
	...
	//初始化合计
	var init_total_money = parseFloat($("#hidden_combo_package_price").val());//商品金额
	//有运费
...
function calculateTotalAmount(){
	var goods_money = parseFloat($("#hidden_count_money").val()); //商品金额
	var money = parseFloat($("#hidden_combo_package_price").val());// 套餐金额
	var total_discount = parseFloat($("#hidden_discount_money").val());//总优惠
	var order_invoice_tax_money = 0;//发票税额 显示
	var tax_sum = parseFloat($("#hidden_combo_package_price").val());//计算发票税额计算：（商品总计+运-优惠活动-优惠券）*发票税率
	var express = 0; //运费
	//商品金额
	if(goods_money > 0){
		$("#js-goods-total-money").text(goods_money.toFixed(2));
	}
	...
	if(getPickupId()>0){
		...
		express = $("#hidden_pick_up_money").val();
		...
		express = $("#hidden_o2o_distribution").val();
	}else{
		//满额包邮
	...
	$("#realprice").attr("data-old-total-money",old_total_money.toFixed(2));//原合计（不包含优惠,但包括税额 减去积分抵现金额）
