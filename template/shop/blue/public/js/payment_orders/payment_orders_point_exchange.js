...
$(function(){
	...
	$(".btn-jiesuan").click(function(){
		if(validationOrder()){
			...
			$.ajax({
				url : __URL(SHOPMAIN + "/order/pointExchangeOrderCreate"),
				type : "post",
				data : {
					...
					'point_exchange_type' : $("#point_exchange_type").val(),
					'order_goods_type' : $("#order_goods_type").val()
				},
...
function calculateTotalAmount(){
	...
	realpriceAndPointDisplay(money, $("#count_point_exchange").val());
...
//显示价格与积分
function realpriceAndPointDisplay(money, point){
	if(money > 0 && point > 0){
		$("#realprice").parent().show();
		$("#and,#count-point").show();
	}else if(money > 0 && point == 0){
		$("#realprice").parent().show();
		$("#and,#count-point").hide();
	}else if(money == 0 && point > 0){
		$("#realprice").parent().hide();
		$("#and").hide();
		$("#count-point").show();
	}
}
