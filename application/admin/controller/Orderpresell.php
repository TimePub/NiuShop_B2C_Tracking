...
use data\service\Orderpresell as OrderpresellService;
...
class Orderpresell extends BaseController
{
    ...
    if (request()->isAjax()) {
            ...
            $order_presell = new OrderpresellService();
...
//             $this->assign("tuangou_group_id", $tuangou_group_id);
            ...
            $all_status = OrderStatus::getOrderPresellStatus();
    ...
    public function presellOrderOffLinePay(){
        if(request()->isAjax()){
            ...
            $res = $order_service->presellOrderOffLinePay($presell_order_id);
    ...
    public function orderDetail()
    {
        ...
        $presell_order = $order_service-> getOrderPresellInfo(0, ['relate_id'=> $order_id]);
    ...
    public function orderStockingComplete(){
        if(request()->isAjax()){
            ...
            $order_service = new OrderService();
            $result = $order_service->setOrderStockingComplete($order_id);
