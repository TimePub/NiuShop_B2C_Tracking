<?php if (!defined('THINK_PATH')) exit(); /*a:9:{s:53:"template/adminblue\Orderpresell\orderPresellList.html";i:1523418002;s:28:"template/adminblue\base.html";i:1522831681;s:45:"template/adminblue\controlCommonVariable.html";i:1522486340;s:32:"template/adminblue\urlModel.html";i:1510819828;s:34:"template/adminblue\pageCommon.html";i:1512444925;s:34:"template/adminblue\openDialog.html";i:1522669943;s:41:"template/adminblue\Order\orderAction.html";i:1524017044;s:47:"template/adminblue\Order\orderRefundAction.html";i:1522665946;s:46:"template/adminblue\Order\orderPrintAction.html";i:1523238980;}*/ ?>
<!DOCTYPE html>
<html>
	<head>
...
function LoadingInfo(page_index) {
	...
	$.ajax({
		type : "post",
		url : "<?php echo __URL('ADMIN_MAIN/orderpresell/orderPresellList'); ?>",
		data : {
...
// 					html +='<span>订单编号：'+order_no+' 交易号：'+out_trade_no+'</span><span>下单时间：'+create_time+'</span>';
					...
					//添加赠品标识
					if(gift_flag > 0){
						html += '<br/><i style="font-size:12px;margin:5px 5px 0 0;padding:1px 4px;border-radius:3px;display:inline-block;color:#FFF;background-color:#FF6600;height:16px;line-height: 16px;overflow:hidden;font-style:normal;">赠品</i>';
					}
					...
					html += '</td>';
					html += '<td rowspan="'+row+'" style="text-align:center">'+ data["data"][i]["presell_money"] +'</td>';
					...
					html += '<td rowspan="'+row+'" style="text-align:center;">';
					html += '<a style="display:block;margin-bottom:5px;" href="'+__URL('ADMIN_MAIN/Orderpresell/orderDetail?order_id='+order_id)+'">订单详情</a>';
					...
					if(data["data"][i]["operation"] != ''){
						for(var m = 0; m < data["data"][i]["operation"].length; m++){
							...
							}else if(data["data"][i]["operation"][m]['no'] == "order_presell"){
								html += '<a style="display:block;margin-bottom:5px;color:'+data["data"][i]["operation"][m]["color"]+'" href="javascript:operation(\''+data["data"][i]["operation"][m]['no']+'\','+data["data"][i]["presell_order_id"]+')" >'+data["data"][i]["operation"][m]['name']+'</a>';
							}else{
								html += '<a style="display:block;margin-bottom:5px;color:'+data["data"][i]["operation"][m]["color"]+'" href="javascript:operation(\''+data["data"][i]["operation"][m]['no']+'\','+data["data"][i]["order_id"]+')" >'+data["data"][i]["operation"][m]['name']+'</a>';
							}
						}
