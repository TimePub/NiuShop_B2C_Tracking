...
<table class="mytable">
	<tr>
		<th width="30%" style="text-align: left;">
			...
			<button class="btn-common" onclick="addBrand()" style="margin:0 5px 0 0 !important;">添加品牌</button>
		</th>
		<th width="20%">
...
function LoadingInfo(page_index) {
	$.ajax({
		type : "post",
		url : "<?php echo __URL('ADMIN_MAIN/goods/goodsbrandlist'); ?>",
		data : {
		...
		success : function(data) {
			if (data["data"].length > 0) {
				...
				for (var i = 0; i < data["data"].length; i++) {
					...
					html += '<td><i class="checkbox-common"><input name="sub" type="checkbox" value="'+ data["data"][i]["brand_id"] +'" ></i></td>';
	...
	$( "#dialog" ).dialog({
		buttons: {
			"确定": function() {
				...
				$.ajax({
					type : "post",
					url : "<?php echo __URL('ADMIN_MAIN/goods/deletegoodsbrand'); ?>",
