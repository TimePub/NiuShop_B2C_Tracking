...
class AppletGoods extends BaseService
{
     ...
     */
    public function addOrEditGoods($goods_id, $goods_name, $shopid, $category_id, $category_id_1, $category_id_2, $category_id_3, $supplier_id, $brand_id, $group_id_array, $goods_type, $market_price, $price, $cost_price, $point_exchange_type, $point_exchange, $give_point, $is_member_discount, $shipping_fee, $shipping_fee_id, $stock, $max_buy, $min_buy, $min_stock_alarm, $clicks, $sales, $collects, $star, $evaluates, $shares, $province_id, $city_id, $picture, $keywords, $introduction, $description, $QRcode, $code, $is_stock_visible, $is_hot, $is_recommend, $is_new, $sort, $image_array, $sku_array, $state, $sku_img_array, $goods_attribute_id, $goods_attribute, $goods_spec_format, $goods_weight, $goods_volume, $shipping_fee_type, $extend_category_id, $sku_picture_values, $virtual_goods_type_id, $production_date, $shelf_life, $ladder_preference, $goods_video_address)
    {
        ...
        if (! empty($cart_goods_list)) {
            foreach ($cart_goods_list as $k => $v) {
                ...
                if ($num != $v['num']) {
                    ...
                    $this->cartAdjustNum($uid, $v['cart_id'], $num);
                }
     ...
     */
    public function cartAdjustNum($uid, $cart_id, $num)
    {
        if ($uid > 0) {
            $cart = new NsCartModel();
            $data = array(
     ...
     */
    public function cartDelete($uid, $cart_id_array)
    {
        if ($uid > 0) {
            $cart = new NsCartModel();
    ...
    public function getGoodsBrowseList($page_index, $page_size, $condition, $order, $field = "*")
    {
        ...
        if (! empty($goods_browse_list)) {
            foreach ($goods_browse_list["data"] as $k => $v) {
                $goods_info = $this->goods->getInfo([
                    "goods_id" => $v["goods_id"]
                ], "category_id, category_id_1, goods_name, promotion_type, promotion_price, shop_id, price, picture, clicks");
     ...
     */
    public function getGoodsQueryLimit($condition, $field, $page_size = PAGESIZE, $order = "ng.sort asc,ng.create_time desc")
    {
        $goods_model = new NsGoodsModel();
     ...
     * @return unknown
     */
    public function getGoodsQueryCount($condition)
    {
        $goods_model = new NsGoodsModel();
    ...
    public function getBackStageGoodsList($page_index = 1, $page_size = 0, $condition = '', $order = 'ng.sort asc')
    {
        ...
        $queryList = $goods_model->viewPageQuery($viewObj, $page_index, $page_size, $condition, "");
    ...
    {
        $json = json_encode($condition);
        $list_cache = Cache::tag("goods_service")->get("get_goods_list_new" . $json . $page_index);
        if (empty($list_cache)) {
            $goods_model = new NsGoodsModel();
            // 针对商品分类
            if (! empty($condition['ng.category_id'])) {
            ...
            Cache::tag("goods_service")->set("get_goods_list_new" . $json . $page_index, $list);
            return $list;
        } else {
            return $list_cache;
        }
