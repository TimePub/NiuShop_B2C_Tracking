...
use data\service\Applet\AppletUser;
use data;
...
class AppletMember extends AppletUser
{
     ...
     */
    public function getDefaultExpressAddress($uid)
    {
        ...
        // 处理地址信息
        if (! empty($data)) {
            $applet_address = new AppletAddress();
            $address_info = $applet_address->getAddress($data['province'], $data['city'], $data['district']);
     ...
     */
    public function getMemberDetail($uid,$shop_id = '')
    {
        // 获取基础信息
        if (! empty($uid)) {
            ...
            // 获取user信息
            $user_info = $this->getUserDetailByUid($uid);
            $member_info['user_info'] = $user_info;
            
            // 获取优惠券信息
            $coupon_list = $this->getUserCouponList('', $shop_id,$uid);
     ...
     */
    public function getUserCouponList($type = '',$shop_id='',$uid)
    {
        $time = time();
        $condition['uid'] = $uid;
        switch ($type)
     ...
     */
    public function addMemberExpressAddress($uid,$consigner, $mobile, $phone, $province, $city, $district, $address, $zip_code, $alias)
    {
        $express_address = new NsMemberExpressAddressModel();
        ...
        $this->updateAddressDefault($uid, $express_address->id);
     ...
     * 修改会员收货地址
     */
    public function updateMemberExpressAddress($uid, $id, $consigner, $mobile, $phone, $province, $city, $district, $address, $zip_code, $alias)
    {
        $express_address = new NsMemberExpressAddressModel();
        $data = array(
            'uid' => $uid,
        ...
        $retval = $this->updateAddressDefault($uid, $id);
    ...
    {
        $express_address = new NsMemberExpressAddressModel();
        $data = $express_address->pageQuery($page_index, $page_size, $condition, 'id desc', '*');
        // 处理地址信息
        if (! empty($data)) {
            foreach ($data['data'] as $key => $val) {
                $applet_address = new AppletAddress();
                $address_info = $applet_address->getAddress($val['province'], $val['city'], $val['district']);
     ...
     */
    public function getMemberExpressAddressDetail($uid, $id)
    {
        $express_address = new NsMemberExpressAddressModel();
     ...
     */
    public function memberAddressDelete($uid, $id)
    {
        $express_address = new NsMemberExpressAddressModel();
        ...
        if ($count == 1) {
            ...
            if ($express_address_info['is_default'] == 1) {
                $express_address_info = $express_address->where(array(
                    ...
                    ->limit(0, 1)
                    ->select();
                $res = $this->updateAddressDefault($uid, $express_address_info[0]['id']);
            }
     ...
     */
    public function updateAddressDefault($uid, $id)
    {
        $express_address = new NsMemberExpressAddressModel();
    ...
    public function getGuessMemberLikes()
    {
        ...
        $goods_list = $goods->getGoodsQueryLimit([
            ...
            'ng.state' => 1
        ], "ng.goods_id,ng.goods_name,ng_sap.pic_cover_mid,ng.price",PAGESIZE);
     ...
     */
    public function delMemberBankAccount($uid, $account_id)
    {
        $member_bank_account = new NsMemberBankAccountModel();
