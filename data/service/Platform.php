...
class Platform extends BaseService implements IPlatform
{
    ...
    {
        $block = new NsPlatformBlockModel();
        $block_list = $this->webBlockList(1, 0, [
            'is_display' => 1
        ], 'sort', 'block_id, block_data');
        $list = array();
        ...
        if (! empty($block_info)) {
            ...
            if (! empty($base_info)) {
                ...
                if (! empty($base_info['recommend_goods_category_1'])) {
                    ...
                    $goods_list_1 = $goods->where('category_id_3 = ' . $base_info['recommend_goods_category_1'])
                        ->limit($select_num)
                        ->field('goods_id,goods_name,price,promotion_price,point_exchange_type,point_exchange,give_point,stock,picture')
                        ->select();
                    if (empty($goods_list_1)) {
                        $goods_list_1 = $goods->where('category_id_2 = ' . $base_info['recommend_goods_category_1'])
                            ->limit($select_num)
                            ->field('goods_id,goods_name,price,promotion_price,point_exchange_type,point_exchange,give_point,stock,picture')
                            ->select();
                    }
                    if (empty($goods_list_1)) {
                        $goods_list_1 = $goods->where('category_id_1 = ' . $base_info['recommend_goods_category_1'])
                            ->limit($select_num)
                            ->field('goods_id,goods_name,price,promotion_price,point_exchange_type,point_exchange,give_point,stock,picture')
                            ->select();
                    }
                    if (! empty($goods_list_1)) {
                        foreach ($goods_list_1 as $k => $v) {
                            ...
                            $pic_info['pic_cover'] = '';
                            if (! empty($v['picture'])) {
                                $pic_info = $picture->getInfo([
                                    'pic_id' => $v['picture']
                                ], 'pic_cover,pic_cover_big,pic_cover_mid,pic_cover_small');
                            }
                ...
                if (! empty($base_info['recommend_goods_category_2'])) {
                    $goods = new NsGoodsModel();
                    $goods_list_2 = $goods->where('category_id_3 = ' . $base_info['recommend_goods_category_2'])
                        ->limit($select_num)
                        ->field('goods_id,goods_name,price,promotion_price,point_exchange_type,point_exchange,give_point,stock,picture')
                        ->select();
                    if (empty($goods_list_2)) {
                        $goods_list_2 = $goods->where('category_id_2 = ' . $base_info['recommend_goods_category_2'])
                            ->limit($select_num)
                            ->field('goods_id,goods_name,price,promotion_price,point_exchange_type,point_exchange,give_point,stock,picture')
                            ->select();
                    }
                    if (empty($goods_list_2)) {
                        $goods_list_2 = $goods->where('category_id_1 = ' . $base_info['recommend_goods_category_2'])
                            ->limit($select_num)
                            ->field('goods_id,goods_name,price,promotion_price,point_exchange_type,point_exchange,give_point,stock,picture')
                            ->select();
                    }
                ...
                if (! empty($base_info['recommend_goods_category_3'])) {
                    $goods = new NsGoodsModel();
                    $goods_list_3 = $goods->where('category_id_3 = ' . $base_info['recommend_goods_category_3'])
                        ->limit($select_num)
                        ->field('goods_id,goods_name,price,promotion_price,point_exchange_type,point_exchange,give_point,stock,picture')
                        ->select();
                    if (empty($goods_list_3)) {
                        $goods_list_3 = $goods->where('category_id_2 = ' . $base_info['recommend_goods_category_3'])
                            ->limit($select_num)
                            ->field('goods_id,goods_name,price,promotion_price,point_exchange_type,point_exchange,give_point,stock,picture')
                            ->select();
                    }
                    if (empty($goods_list_3)) {
                        $goods_list_3 = $goods->where('category_id_1 = ' . $base_info['recommend_goods_category_3'])
                            ->limit($select_num)
                            ->field('goods_id,goods_name,price,promotion_price,point_exchange_type,point_exchange,give_point,stock,picture')
                            ->select();
                    }
    ...
    {
        Cache::tag("niu_platform_adv_position")->set("getPlatformAdvPositionDetail" . $ap_id, null);
        $platform_adv = new NsPlatformAdvModel();
        $data = array(
            'ap_id' => $ap_id,
            'adv_title' => $adv_title,
    ...
    {
        $cache = Cache::tag("niu_platform_adv_position")->get("getPlatformAdvPositionDetail" . $ap_id);
        if (empty($cache)) {
            $platform_adv_position = new NsPlatformAdvPositionModel();
            ...
            Cache::tag("niu_platform_adv_position")->set("getPlatformAdvPositionDetail" . $ap_id, $info);
            return $info;
        } else {
            return $cache;
        }
    ...
    {
        Cache::tag("niu_platform_help")->clear();
        $platform_class = new NsPlatformHelpClassModel();
    ...
    public function getPlatformGoodsRecommend($type)
    {
        ...
        $list = $platform_goods_recommend->getQuery([
            'class_id' => $type
        ], '*', 'create_time desc');
        if (! empty($list))
            foreach ($list as $k => $v) {
    ...
    {
        $platform_goods_recommend = new NsPlatformGoodsRecommendModel();
        $count = $platform_goods_recommend->where([
            'goods_id' => $goods_id,
            'class_id' => 2
        ])->count();
        return $count;
    ...
    public function modifyGoodsRecommend($goods_id, $type, $is_recommend)
    {
        ...
        if ($is_recommend == 1) {
            ...
            if ($count > 0) {
                $retval = $platform_goods_recommend->save([
                    'state' => 1
                ], [
                    'goods_id' => $goods_id,
                    'class_id' => $type
            ...
            $platform_goods_recommend = new NsPlatformGoodsRecommendModel();
            $retval = $platform_goods_recommend->destroy([
                'goods_id' => $goods_id,
                'class_id' => $type
            ]);
        }
    ...
    public function updatePlatformGoodsRecommendClass($class_id, $class_name, $sort, $goods_id_array, $show_type)
    {
        ...
        try {
            if ($class_id > 0) {
                ...
                // 先删除掉已有的
                $platform_goods_recommend = new NsPlatformGoodsRecommendModel();
                $platform_goods_recommend->destroy([
                    'class_id' => $class_id
                ]);
        ...
        try {
            if ($class_id > 0) {
                ...
                if ($goods_id_old > 0 && $goods_id > 0) {
                    ...
                    $res = $platform_goods_recommend->save($data, [
                        'class_id' => $class_id,
                        'goods_id' => $goods_id_old
                    ]);
                } else 
                    if ($goods_id > 0 && empty($goods_id_old)) {
                        $data = array(
                            'goods_id' => $goods_id,
                            'state' => 1,
                            'class_id' => $class_id,
    ...
    public function getPlatformCount($start_date, $end_date)
    {
        ...
        // 订单总额
        $order_total = $account_records->where(array(
            "create_time" => array(
                array(
                    ">",
                    $start_date
        ...
        // 提现
        $cash_withdrawal_amount = $account_records->where(array(
            "create_time" => array(
                array(
                    ">",
                    $start_date
        ...
        // 平台余额
        $platform_balance = $accountReturnRecords->where(array(
            "create_time" => array(
                array(
                    ">",
                    $start_date
    ...
    public function getShopSalesVolume($condition)
    {
        ...
        foreach ($shop_list as $k => $v) {
            ...
            $shop_sales_volume = $shop_account_record->where(array(
                "shop_id" => $v["shop_id"],
                "account_type" => 1,
    ...
    public function getGoodsGroupRecommend($shop_id)
    {
        ...
        $info = $platform_goods_group_recommend->getInfo([
            "shop_id" => $shop_id
        ], "*");
        if (empty($info)) {
            $data = [
    ...
    {
        // TODO Auto-generated method stub
        // $cache = Cache::tag("niu_goods_group")->get("getRecommendGoodsQuery".$shop_id);
        // if(empty($cache))
        // {
        ...
        $group_list = $group->getQuery([
            "shop_id" => $shop_id
        ], "group_id,group_pic,group_name", "sort asc");
        $picture = new AlbumPictureModel();
        foreach ($group_list as $k => $v) {
          ...
          //  $goods_list = $goods_service->getRecommendGoodsQuery(1, PAGESIZE, "FIND_IN_SET(" . $v["group_id"] . ",ng.group_id_array) AND ng.state = 1");
        ...
        // print_r(json_encode($group_goods_list));
        // Cache::tag("niu_goods_group")->set("getRecommendGoodsQuery".$shop_id, $group_goods_list);
